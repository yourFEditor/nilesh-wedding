        ...
        {eventImage && (
          <div className={`flex items-center justify-center flex-1 z-10 ${day.title.toLowerCase().includes('lagan') ? 'py-4 sm:py-5' : ''}`}>
            <img 
              src={eventImage} 
              alt={day.title}
              className={`object-contain drop-shadow-lg ${day.title.toLowerCase().includes('lagan') ? 'w-28 h-28 sm:w-32 sm:h-32 md:w-36 md:h-36' : 'w-32 h-32 sm:w-36 sm:h-36 md:w-40 md:h-40'}`}
            />
          </div>
        )}
